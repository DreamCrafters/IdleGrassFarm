...
public class PlayerMowPresenter : IInitializable, IDisposable, ITickable
{
    ...
    public void Tick()
    {
        foreach (FarmFieldTileModel tile in _processingTiles)
        {
            if (tile.CurrentStage == TileStage.Stage3)
            {
                tile.SetStage(TileStage.Stage0);
                _playerModel.AddCorns(tile.CornsSpawnPoint.position);
            }
        }
    }
    ...
    private void OnTriggerExit(Collider other)
    {
        if (other.CompareTag(FarmTag)
            && other.TryGetComponent(out FarmFieldTileModel farmFieldTile))
        {
